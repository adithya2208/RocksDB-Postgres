...
static const f_smgr smgrsw[] = {
	...
	/*
	 * CS448: YOUR CODE HERE
	 */
	{
		RDBinit,NULL,RDBclose,RDBcreate,RDBexists,RDBunlink,RDBextend,
		RDBprefetch,RDBread,RDBwrite,RDBnblocks,RDBtruncate,RDBimmedsync,
		NULL,NULL,NULL
	}
};
...
smgropen(RelFileNode rnode, BackendId backend)
{
	...
	if (!found)
	{
		...
		/* CS448:
		 * Check if this is a RocksDB relation. If so, select our RocksDB
		 * storage manager. We don't have access to relation name here so we
		 * can't tell if this is a RocksDB relation other than to check if
		 * it's in RocksDB already.
		 * See also RelationOpenSmgr() in rel.h.
		 */
		if (RDBexists(reln, MAIN_FORKNUM)){
			reln->smgr_which = 1;
		} else {
			reln->smgr_which = 0;
		}
